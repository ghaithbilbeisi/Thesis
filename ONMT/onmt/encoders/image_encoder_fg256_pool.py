...
class ImageEncoder(EncoderBase):
    ...
    def forward(self, src, lengths=None):
        ...
        # max pool 1 (batch_size, 64, imgH/2/2, imgW/2/2)
        src = F.max_pool2d(src, kernel_size=(2, 2), stride=(2, 2))
        # batch norm 1
        src = self.batch_norm1(src)

        # (batch_size, 64, imgH/2/2, imgW/2/2)
        # Block 2
        ...
        # max pool 2 (batch_size, 128, imgH/2/2, imgW/2/2/2)
        src = F.max_pool2d(src, kernel_size=(2, 1), stride=(2, 1))
        # batch norm 2 (batch_size, 128, imgH/2/2, imgW/2/2/2)
        src = self.batch_norm2(src)

        # (batch_size, 128, imgH/2/2, imgW/2/2/2)
        # Block 3
        ...
        # max pool 3 (batch_size, 128, imgH/2/2/2, imgW/2/2/2)
        src = F.max_pool2d(src, kernel_size=(1,2), stride=(1,2))
        # batch norm 3 (batch_size, 128, imgH/2/2/2, imgW/2/2/2)
        src = self.batch_norm3(src)

        # # (batch_size, 128, H/2/2/2, W/2/2/2)
        all_outputs = []
