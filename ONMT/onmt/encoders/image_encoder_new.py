...
class ImageEncoder(EncoderBase):
    ...
    Args:
        ...
        antialiasing (int): antialiasing mode (0, 1, 2, 3, or 4).
        blur_kernel_size (int): size of the blur kernel for antialiasing (1, 2, 3, 4, 5, 6, or 7).
    """

    def __init__(self, num_layers, bidirectional, rnn_size, dropout,
                 image_chanel_size=3, antialiasing=0, blur_kernel_size=3):
        ...
        self.ds1 = Downsample(channels=64, filt_size=blur_kernel_size, stride=2)
        self.layer2 = nn.Conv2d(64, 128, kernel_size=(3, 3),
                                padding=(1, 1), stride=(1, 1))
        self.ds2 = Downsample(channels=128, filt_size=blur_kernel_size, stride=2)
        ...
        if antialiasing != 0:
            ...
            self.ds2 = Downsample(channels=128, filt_size=blur_kernel_size, stride=2)
            if antialiasing > 1:
                self.ds4 = Downsample(channels=256, filt_size=blur_kernel_size, stride=2)
                self.ds5 = Downsample(channels=512, filt_size=blur_kernel_size, stride=2)
            if antialiasing == 3:
                self.ds5 = Downsample(channels=512, filt_size=blur_kernel_size, stride=2)
